...
public class HomePresenter extends BasePresenter<IHome.View> implements IHome.IPresenter {
    @Override
    public void getData() {
        addDisposable(HttpMessage.getInstance().getiHomeApi().getHomeContent()
        ...
        .subscribeWith(new CommonSubscriber<List<HomeBean.HomeListBean>>(mView) {
            @Override
            public void onNext(List<HomeBean.HomeListBean> homeBeans) {
                mView.getData(homeBeans);
            }
        }));
    }
}
